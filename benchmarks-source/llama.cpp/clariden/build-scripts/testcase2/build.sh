...
#SBATCH --output=build_llama_container.out
#SBATCH --error=build_llama_container.err
...
cp -r ${ARTIFACT_LOCATION}/data/llama.cpp/llama.cpp .
...
ENVIRONMENT_FILE=${ARTIFACT_LOCATION}/benchmarks-source/llama.cpp/clariden/build-scripts/testcase2/environment.toml
echo "Create new ${ENVIRONMENT_FILE}"
cat > ${ENVIRONMENT_FILE} << EOF
image = "${ARTIFACT_LOCATION}/benchmarks-source/llama.cpp/clariden/build-scripts/testcase2/${CONTAINER_IMAGE}"
workdir = "${SCRATCH}"
writable = true
...
podman login docker.io -u XXXXX -p XXXXX
