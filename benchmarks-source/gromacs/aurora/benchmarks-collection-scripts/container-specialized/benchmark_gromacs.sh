...
TESTCASE_DIR="${ARTIFACT_LOCATION}/benchmarks-source/gromacs/aurora/gromacs-benchmarks/TestcaseB_benchmarks/gromacs_specialized_testcaseB"
...
for i in $(seq 1 $TOTAL_RUNS); do
  ...
  apptainer exec ${CONTAINER_PATH} gmx mdrun -s $TPR_FILE -ntomp 104 -ntmpi 1 -nsteps 100 > "$RUN_DIR/mdrun_output.log" 2>&1
