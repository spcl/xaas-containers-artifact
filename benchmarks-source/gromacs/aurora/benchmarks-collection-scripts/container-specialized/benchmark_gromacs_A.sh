...
TESTCASE_DIR="${ARTIFACT_LOCATION}/benchmarks-source/gromacs/aurora/gromacs-benchmarks/TestcaseA_benchmarks/gromacs_specialized_testcaseA/steps_${STEPS}"
TPR_FILE="${ARTIFACT_LOCATION}/data/gromacs/GROMACS_TestCaseA/ion_channel.tpr"

CONTAINER_PATH="${ARTIFACT_LOCATION}/data/gromacs/images/gromacs-mpi-ipc.sing"
...
apptainer exec ${CONTAINER_PATH} which gmx
apptainer exec ${CONTAINER_PATH} ldd $(which gmx)
apptainer exec ${CONTAINER_PATH} gmx --version
...
for i in $(seq 1 $TOTAL_RUNS); do
  ...
  apptainer exec ${CONTAINER_PATH} gmx mdrun -s $TPR_FILE -ntomp 104 -ntmpi 1 -nsteps ${STEPS} -resethway >"$RUN_DIR/mdrun_output.log" 2>&1
