...
TESTCASE_DIR="${ARTIFACT_LOCATION}/benchmarks-source/gromacs/aurora/gromacs-benchmarks/TestcaseA_benchmarks/gromacs_native_testcaseA/steps_${STEPS}"
...
for i in $(seq 1 $TOTAL_RUNS); do
  ...
  gmx_mpi mdrun -s "$TPR_FILE" -ntomp 104 -nsteps ${STEPS} -resethway >"$RUN_DIR/mdrun_output.log" 2>&1
