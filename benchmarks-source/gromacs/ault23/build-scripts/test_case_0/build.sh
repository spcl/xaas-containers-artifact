...
cd ${ARTIFACT_LOCATION}/benchmarks-source/gromacs/ault23/build-scripts/test_case_0

rm -rf build && mkdir -p build
mkdir -p install

# build command
cd build && ${ARTIFACT_LOCATION}/data/gromacs/cmake/bin/cmake ${ARTIFACT_LOCATION}/data/gromacs/gromacs-2025.0 -DGMX_BUILD_OWN_FFTW=ON -DREGRESSIONTEST_DOWNLOAD=ON -DCMAKE_INSTALL_PREFIX=$(pwd)/../install
