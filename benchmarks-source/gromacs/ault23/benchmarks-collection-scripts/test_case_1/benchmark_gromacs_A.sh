...
source ${ARTIFACT_LOCATION}/benchmarks-source/gromacs/ault23/build-scripts/test_case_1/build/bin/GMXRC

export OMP_NUM_THREADS=64     # 16 OpenMP threads per MPI process
export CUDA_VISIBLE_DEVICES=0 # Ensure GPU visibility

TESTCASE_DIR="${ARTIFACT_LOCATION}/benchmarks-source/gromacs/ault23/gromacs-benchmarks/TestcaseA_benchmarks/gromacs_testcase1_testcaseA/steps_${STEPS}"
...
for i in $(seq 1 $TOTAL_RUNS); do
  ...
  pushd ${RUN_DIR}
  mpirun -np 1 ${ARTIFACT_LOCATION}/benchmarks-source/gromacs/ault23/build-scripts/test_case_1/build2/bin/gmx_mpi mdrun -s "$TPR_FILE" -ntomp 64 -gpu_id 0 -nsteps ${STEPS} -resethway >"$RUN_DIR/mdrun_output.log" 2>&1
  popd
