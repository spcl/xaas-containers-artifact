...
TESTCASE_DIR="${ARTIFACT_LOCATION}/benchmarks-source/gromacs/ault23/gromacs-benchmarks/TestcaseA_benchmarks/gromacs_testcase3_testcaseA/steps_${STEPS}"
...
for i in $(seq 1 $TOTAL_RUNS); do
  ...
  srun --mpi=pmi2 sarus run --mpi \
    ...
    cd \"/result_dir/run${i}\" && \
    gmx_mpi mdrun -s \"/input_dir/ion_channel.tpr\" -ntomp 64 -nsteps ${STEPS} -resethway > mdrun_output.log 2>&1"
