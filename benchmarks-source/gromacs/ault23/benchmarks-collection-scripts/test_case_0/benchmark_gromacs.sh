...
# Define paths
TESTCASE_DIR="${ARTIFACT_LOCATION}/benchmarks-source/gromacs/ault23/gromacs-benchmarks/TestcaseB_benchmarks/gromacs_testcase0_testcaseB/steps_${STEPS}"
...
for i in $(seq 1 $TOTAL_RUNS); do
  ...
  pushd ${RUN_DIR}
  gmx mdrun -s "$TPR_FILE" -ntmpi 1 -ntomp 64 -nsteps ${STEPS} >"$RUN_DIR/mdrun_output.log" 2>&1
  popd
