...
DOCKER_IMAGE="gromacs-source-deploy-ault23_second"
STEPS=20000
...
TESTCASE_DIR="${ARTIFACT_LOCATION}/benchmarks-source/gromacs/ault23/gromacs-benchmarks/TestcaseA_benchmarks/gromacs_testcase2_testcaseA/steps_${STEPS}"
...
for i in $(seq 1 $TOTAL_RUNS); do
  ...
  srun --mpi=pmi2 sarus run --mpi \
    ...
    ${DOCKER_REPOSITORY}:${DOCKER_IMAGE} bash -c "
      ...
      gmx_mpi mdrun -s \"/input_dir/ion_channel.tpr\" -ntomp 64 -nsteps ${STEPS} -resethway > \"/result_dir/run${i}/mdrun_output.log\" 2>&1 && \
  ...
  #ealnuaimi/testcase_2:latest bash -c "
  #source /usr/local/gromacs/bin/GMXRC && \
  #export OMP_NUM_THREADS=64 && \
  #export HYDRA_LAUNCHER=fork && \
  #export CUDA_VISIBLE_DEVICES=0 &&  # Ensure GPU 0 is used
  #mkdir -p \"/host_home/benchmarks_phase1/TestcaseA_benchmarks/gromacs_testcase2_testcaseA/run${i}\" && \
  #mpiexec -n 1 gmx_mpi mdrun -s \"/host_home/GROMACS_TestCaseA/ion_channel.tpr\" -ntomp 64  -nsteps 100 > \"/host_home/benchmarks_phase1/TestcaseA_benchmarks/gromacs_testcase2_testcaseA/run${i}/mdrun_output.log\" 2>&1 && \
  #mv md.log traj* ener.edr confout.gro state.cpt \"/host_home/benchmarks_phase1/TestcaseA_benchmarks/gromacs_testcase2_testcaseA/run${i}/\" 2>/dev/null
  #"
