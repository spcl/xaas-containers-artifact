...
#SBATCH --nodelist=ault25                # Assign the job to ault24
#SBATCH --ntasks=1                        # 4 MPI processes
#SBATCH --cpus-per-task=256                # 16 OpenMP threads per MPI process
#SBATCH --gpus=1                          # 1 GPU
...
DOCKER_IMAGE="gromacs-source-deploy-ault25"
...
TESTCASE_DIR="${ARTIFACT_LOCATION}/benchmarks-source/gromacs/ault23/gromacs-benchmarks/TestcaseA_benchmarks-portable/gromacs_testcase0_ault25_testcaseA/steps_${STEPS}"
...
for i in $(seq 1 $TOTAL_RUNS); do
  ...
  srun --mpi=pmi2 sarus run --mpi \
    ...
    cd \"/result_dir/run${i}\" && \
    gmx_mpi mdrun -s \"/input_dir/ion_channel.tpr\" -ntomp 128 -nsteps ${STEPS} -resetstep 10000 > mdrun_output.log 2>&1"
