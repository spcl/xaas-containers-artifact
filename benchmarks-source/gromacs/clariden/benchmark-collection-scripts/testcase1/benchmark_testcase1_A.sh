...
source ${ARTIFACT_LOCATION}/benchmarks-source/gromacs/clariden/build-scripts/testcase1/install/bin/GMXRC

TPR_FILE="${ARTIFACT_LOCATION}/data/gromacs/GROMACS_TestCaseA/ion_channel.tpr"
BENCH_DIR="${ARTIFACT_LOCATION}/benchmarks-source/gromacs/clariden/gromacs-benchmarks/TestcaseA_benchmarks/gromacs_testcase1_testcaseA/steps_${STEPS}"
GROMACS_BIN="${ARTIFACT_LOCATION}/benchmarks-source/gromacs/clariden/build-scripts/testcase1/install/bin/gmx_mpi" 
...
for i in $(seq 1 $TOTAL_RUNS); do
    ...
    #srun --cpu-bind=socket "$MPS_WRAPPER" "$GROMACS_BIN" mdrun -s "$TPR_FILE" -ntomp 64 -bonded gpu -nb gpu -pme gpu -pin on -v -noconfout -dlb yes -nstlist 300 -gpu_id 0 -npme 1 -nsteps 300 > mdrun_output.log 2>&1
