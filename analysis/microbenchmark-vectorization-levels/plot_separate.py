    ...
    for arch in ["intel", "arm"]:
        ...
        for vec_flag in os.listdir(arch_path):
            ...
            if wall_times and performances:
                ...
                print(
                    f"{label} → Execution Time: {wall_mean:.2f} ± {wall_ci:.2f} s | Performance: {perf_mean:.2f} ± {perf_ci:.2f} ns/day"
                )
...
def plot_vectorization_results(results, title_prefix, filter_set):
    ...
    labels = [
        item[0].split("-", 1)[1] for item in sorted_items
    ]  # remove 'intel-' or 'arm-' prefix for display
