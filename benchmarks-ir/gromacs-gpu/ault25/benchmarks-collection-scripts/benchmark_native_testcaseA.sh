...
TESTCASE_DIR="${ARTIFACT_LOCATION}/benchmarks-ir/gromacs-gpu/ault25/gromacs-benchmarks/TestcaseA_benchmarks/gromacs_native_testcaseA/steps_${STEPS}"
...
for i in $(seq 1 $TOTAL_RUNS); do
  ...
  pushd ${RUN_DIR}
  srun ${ARTIFACT_LOCATION}/benchmarks-source/gromacs/ault23/build-scripts/test_case_1/build2/bin/gmx_mpi mdrun -s "$TPR_FILE" -ntomp 128 -gpu_id 0 -nsteps ${STEPS} -resethway >"$RUN_DIR/mdrun_output.log" 2>&1
  popd
