...
#SBATCH --cpus-per-task=128                        # 4 MPI processes
...
ARCH_TYPE="gromacs-gpu-specialized-container-avx2-256"
TESTCASE_DIR="${ARTIFACT_LOCATION}/benchmarks-ir/gromacs-gpu/ault25/gromacs-benchmarks/TestcaseA_benchmarks/gromacs_specialized_testcaseA/steps_${STEPS}"
TPR_FILE="${ARTIFACT_LOCATION}/data/gromacs/GROMACS_TestCaseA/ion_channel.tpr"
CONTAINER_IMAGE="${DOCKER_REPOSITORY}:${ARCH_TYPE}"
...
for i in $(seq 1 $TOTAL_RUNS); do
    ...
    cd \"/scratch/run${i}\" && \
    /build/bin/gmx mdrun -s \"/input_dir/ion_channel.tpr\" -ntomp ${OMP_NUM_THREADS} -ntmpi 1 -nsteps ${STEPS} -resethway > \"/scratch/run${i}/mdrun_output.log\" 2>&1"
