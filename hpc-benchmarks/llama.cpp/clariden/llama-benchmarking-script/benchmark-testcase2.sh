...
# srun -A a-g200 --ntasks=1 --cpus-per-task=64 --hint=nomultithread --time=01:00:00 --environment=llamacpp-testcase2 --pty bash
...
CUDA_VISIBLE_DEVICES=0 ./llama-bench \
  ...
  -t "$OMP_NUM_THREADS" \
  -r 40 \
  -o csv > "$OUTPUT_DIR/Q4_K_M/testcase2_pg_results.csv"
...
CUDA_VISIBLE_DEVICES=0 ./llama-bench \
  ...
  -t "$OMP_NUM_THREADS" \
  -r 40 \
  -o csv > "$OUTPUT_DIR/Q4_0/testcase2_pg_results.csv"