...
TESTCASE_DIR="$HOME/xaas-containers-artifact/hpc-benchmarks/gromacs/aurora/gromacs-benchmarks/TestcaseB_benchmarks/gromacs_source-container_testcaseB"
...
for i in $(seq 1 $TOTAL_RUNS); do
  ...
  apptainer exec $HOME/xaas-containers/images/apptainer/gromacs-xaas-source.sing /bin/bash -c "source /usr/local/gromacs/bin/GMXRC && gmx mdrun -s $TPR_FILE -ntomp 104 -ntmpi 1 -nsteps 100" > "$RUN_DIR/mdrun_output.log" 2>&1
