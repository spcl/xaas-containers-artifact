...
class ClaudeInterface:
    ...
    def __init__(self):
        ...
        if not self.api_key:
            raise EnvironmentError("Error: ANTHROPIC_API_KEY environment variable not set.")
...
class SpecializationBatchExtractor:
    def __init__(
        self,
        project_dir,
        output_dir="claude-3-7-thinking",
    ...
    def get_prompt(self):
        cmake_path = self.helper.find_build_files(self.project_dir)
        file_content = self.helper.read_file(cmake_path)
        return self.helper.PROMPT_TEMPLATE.format(
            schema=json.dumps(self.helper.schema),
    ...
    def run(self, num_runs=10):
        ...
        for i in range(1, num_runs + 1):
            ...
            response_stream = self.client.beta.messages.create(
            ...
            for chunk in response_stream:
                if chunk.type == "content_block_delta":
                    response_text += chunk.delta.text or ""

            elapsed_time = time.time() - start_time

            # Clean up JSON fencing if present
